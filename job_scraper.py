...
============================================================
  ...
  Kutuphane: curl_cffi (TLS/JA3 fingerprint bypass)
============================================================
...
# ── curl_cffi (dokumantasyon: github.com/lexiforest/curl_cffi) ──
try:
...
# Cloudflare bypass için gerekli gelişmiş tarayıcı başlıkları
HEADERS_INDEED = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36',
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    'Accept-Language': 'en-US,en;q=0.9',
    'Accept-Encoding': 'gzip, deflate, br',
    'DNT': '1',  # Do Not Track'Connection': 'keep-alive',
...
def indeed_ara(arama: str) -> list:
    """
    Indeed.com Turkiye is ilanlarini ceker.

    curl_cffi kutuphanesi Chrome'un TLS parmak izini (JA3/JA4) birebir
    taklit eder. Cloudflare ve benzeri bot korumalarini bu sekilde aser.

    Dokumantan kullanimi (requests-like API):
    ...
    # ── Session kullanimi (cookie'leri otomatik saklar) ──
    # Dokumantan: s = curl_cffi.Session()
    #             s.get(url, impersonate="chrome")
    session = curl_requests.Session()

    # Adim 1: Oncelikle Indeed ana sayfasini ziyaret et
    # Bu gercek bir kullanici gibi davranmamizi saglar
...
def main():
    ...
    print("  curl_cffi v" + curl_requests.__version__ if hasattr(curl_requests, '__version__') else "  curl_cffi")
    print("=" * 60)
    ...
    if ilanlar:
        print(f"\n{'='*60}")
        print(f"  Toplam {len(ilanlar)} is ilani bulundu | Indeed.com Turkiye")
        print(f"{'='*60}\n")
